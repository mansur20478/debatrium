...
if [[ -z "$DEBATE_VPC" || "$DEBATE_VPC" == "None" ]]; then
    echo "ERROR: Debate VPC '${VPC_NAME}' not found. Run setup.py first."
    exit 1
fi
...
if [[ "$TEMPLATE_EXISTS" == "None" || -z "$TEMPLATE_EXISTS" ]]; then
    ...
    echo "       Run create-judge-launch-template.sh first."
    exit 1
fi
...
if [[ "$EXISTING_ASG" -gt "0" ]]; then
    ...
    echo "To recreate: aws autoscaling delete-auto-scaling-group --auto-scaling-group-name ${ASG_NAME} --force-delete"
else
    echo ""
    ...
    aws autoscaling create-auto-scaling-group \
        ...
        --tags \
            Key=Name,Value=JudgeAgent,PropagateAtLaunch=true \
            Key=Environment,Value=Lab,PropagateAtLaunch=true \
            Key=Role,Value=judge-agent,PropagateAtLaunch=true \
...
for i in $(seq 1 24); do
    ...
    if [[ "$i" -eq 24 ]]; then
        echo "WARNING: Timed out. Check EC2 console."
    fi
    sleep 15
done
...
echo "  Lenses     : fairness | accuracy | consensus"
echo "============================================================"